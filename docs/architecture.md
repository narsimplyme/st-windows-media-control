...
- Metadata reads are bounded and run independently of audio. Source resolves `SourceAppUserModelId` through Windows `AppInfo.DisplayInfo.DisplayName` on Windows 10 2004+; unresolved apps and older Windows retain the model ID. Resolution is cached for the current session. There is no player-specific friendly-name database. Text is capped at 256 Unicode scalar values. Optional GSMTC thumbnails are bounded to 1 MB and a two-second read deadline. Only the current converted JPEG is retained in memory; a fixed /v1/artwork/cover.jpg URL is available on the bound interface's local subnet without phone-IP registration. No external image service is used.
